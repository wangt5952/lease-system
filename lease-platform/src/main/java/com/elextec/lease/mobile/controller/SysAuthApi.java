...
public class SysAuthApi extends BaseController {
    ...
    public MessageResponse register(@RequestBody String userInfo){
        // 无参数则报“无参数”
        if (WzStringUtil.isBlank(userInfo)) {
            ...
            try{
                ...
                if(!WzStringUtil.isNotBlank(resetParam.getLoginName())){
                    ...
                    resetParam.setLoginName(resetParam.getUserMobile());
                }
                //手机注册默认是个人帐户
                resetParam.setUserType(OrgAndUserType.INDIVIDUAL);
                //默认状态为正常
                resetParam.setUserStatus(RecordStatus.NORMAL);
                //默认暂时未实名认证
                resetParam.setUserRealNameAuthFlag(RealNameAuthFlag.AUTHORIZED);
                sysUserService.insertSysUser(resetParam);
                return new MessageResponse(RunningResult.SUCCESS);
            }catch(Exception ex){
                throw new BizException(RunningResult.PARAM_ANALYZE_ERROR, ex);
            }
