...
public class BizOrgController extends BaseController {
     ...
     * </pre>
     */
    @RequestMapping(value = "/getOrgIdByVehicle",method = RequestMethod.POST)
    public MessageResponse getOrgIdByVehicle(@RequestBody String id,HttpServletRequest request){
        // 无参数则报“无参数”
        if (WzStringUtil.isBlank(id)){
            ...
            try {
                String ids = URLDecoder.decode(id, "utf-8");
                map = JSONObject.parseObject(ids,Map.class);
                ...
                //如果是企业只能查看该企业下的所有车辆
                //如果是个人则无权查看
                if (sysUser.getUserType().equals(OrgAndUserType.PLATFORM)){
                    return new MessageResponse(RunningResult.SUCCESS,bizOrganizationService.getOrgIdByVehicle(null,map.get("orgId").toString()));
                } else if (sysUser.getUserType().equals(OrgAndUserType.ENTERPRISE)) {
                    return new MessageResponse(RunningResult.SUCCESS,bizOrganizationService.getOrgIdByVehicle(sysUser.getId(),map.get("orgId").toString()));
                } else if (sysUser.getUserType().equals(OrgAndUserType.INDIVIDUAL)) {
                    return new MessageResponse(RunningResult.NO_FUNCTION_PERMISSION);
                } else {
                    return new MessageResponse(RunningResult.PARAM_ANALYZE_ERROR);
                }
